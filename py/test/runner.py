...
class LuciferQuotesTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            LuciferQuotesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = LuciferQuotesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = LuciferQuotesTestRunner.getenv("LUCIFER_QUOTES_TEST_LIVE")
        override = LuciferQuotesTestRunner.getenv("LUCIFER_QUOTES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = LuciferQuotesTestRunner.getenv(key)
        ...
        explain = LuciferQuotesTestRunner.getenv("LUCIFER_QUOTES_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if LuciferQuotesTestRunner._test_control is not None:
            return LuciferQuotesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                LuciferQuotesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            LuciferQuotesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = LuciferQuotesTestRunner.load_test_control()
...
def load_env_local():
    LuciferQuotesTestRunner.load_env_local()


def env_override(m):
    return LuciferQuotesTestRunner.env_override(m)


def entity_data(v):
    return LuciferQuotesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return LuciferQuotesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return LuciferQuotesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return LuciferQuotesTestRunner.load_test_control()


def live_client_options():
    return LuciferQuotesTestRunner.live_client_options()


def live_delay_ms():
    return LuciferQuotesTestRunner.live_delay_ms()
