...
//     (\`new LuciferQuotesSDK(...)\` / \`LuciferQuotesSDK.test(...)\` become
//     \`LuciferQuotesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'LuciferQuotesSDK'
...
// rewrite real client construction (\`new LuciferQuotesSDK(...)\` and any
// \`LuciferQuotesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+LuciferQuotesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/LuciferQuotesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
