...
`client.Quote()` — each with a small set of operations (`load`)
...
| `tester(testopts?, sdkopts?)` | `LuciferQuotesSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `LuciferQuotesSDK.test(testopts?, sdkopts?)` | `LuciferQuotesSDK` | Create a test-mode client. |
...
| `client` | `client(): LuciferQuotesSDK` | Return the parent SDK client. |
...
lucifer-quotes/
├── src/
│   ├── LuciferQuotesSDK.ts        # Main SDK class
