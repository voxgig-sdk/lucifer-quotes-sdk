# LuciferQuotes SDK Quote entity
...
class QuoteEntity
  ...
  def data_set(args)
    if args
      @_data = LuciferQuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = LuciferQuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Quote, Hash] the loaded Quote; raises LuciferQuotesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = LuciferQuotesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
