...
require_relative 'LuciferQuotes_types'
...
class LuciferQuotesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = LuciferQuotesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = LuciferQuotesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, LuciferQuotesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    LuciferQuotesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = LuciferQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = LuciferQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = LuciferQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => LuciferQuotesError.new(
        "#{op}_allow",
        "LuciferQuotesSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = LuciferQuotesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = LuciferQuotesError.new(
        "graphql_error", "LuciferQuotesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = LuciferQuotesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
