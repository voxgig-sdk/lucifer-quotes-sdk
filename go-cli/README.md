...
boru-driven command-line client **and** interactive REPL for the LuciferQuotes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/lucifer-quotes-cli)
make build

# 2. See usage (words, entities, env vars)
./lucifer-quotes-cli --help

# 3. Provide credentials once, via the environment
export LUCIFER_QUOTES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./lucifer-quotes-cli load 1 quote            # {id:1} shorthand
./lucifer-quotes-cli load '{id:1}' quote       # explicit match map

# 5. Override the API base URL for a single call
LUCIFER_QUOTES_BASE=https://api.example.com ./lucifer-quotes-cli load 1 quote

# 6. No arguments -> interactive REPL
./lucifer-quotes-cli
lucifer-quotes> load 1 quote
lucifer-quotes> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/lucifer-quotes-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/lucifer-quotes-cli load 1 quote
   ```
...
```sh
./lucifer-quotes-cli load 1 quote          # scalar shorthand for {id:1}
./lucifer-quotes-cli load '{id:1}' quote     # explicit match map
```
...
./lucifer-quotes-cli load 1 quote
```
...
Run with no arguments to open a REPL (prompt `lucifer-quotes>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/lucifer-quotes-cli`. |
...
(`./lucifer-quotes-cli <expr>`) and interactively (the REPL), and expressions compose the
