...
// LuciferQuotes SDK feature test
//
...
class FtCtrl extends LuciferQuotesControl
{
...
class FtClient
{
    ...
    private LuciferQuotesUtility $utility;

    public function __construct(LuciferQuotesUtility $utility, array $options)
    {
    ...
    public function get_utility(): LuciferQuotesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LuciferQuotesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LuciferQuotesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LuciferQuotesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LuciferQuotesConfig::make_feature($name);
    ...
    private static function buildUrl(LuciferQuotesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LuciferQuotesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LuciferQuotesResponse($fetched);
        }
    ...
    private function fail(LuciferQuotesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LuciferQuotesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
