...
// LuciferQuotes SDK pipeline test
//
...
require_once __DIR__ . '/../luciferquotes_sdk.php';
...
{
    private static function utility(): LuciferQuotesUtility
    {
        return new LuciferQuotesUtility();
    }
    ...
    private static function ctx(array $over = []): LuciferQuotesContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new LuciferQuotesContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new LuciferQuotesOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof LuciferQuotesError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new LuciferQuotesOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = LuciferQuotesMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new LuciferQuotesOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = LuciferQuotesMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new LuciferQuotesOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = LuciferQuotesMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new LuciferQuotesSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = LuciferQuotesMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new LuciferQuotesResponse([]),
            'result' => new LuciferQuotesResult([]),
        ]);
        [, $err] = LuciferQuotesMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new LuciferQuotesSpec([]),
            'response' => null,
            'result' => new LuciferQuotesResult([]),
        ]);
        [, $err] = LuciferQuotesMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new LuciferQuotesSpec(['step' => 's']),
            'response' => new LuciferQuotesResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new LuciferQuotesResult([]),
        ]);
        [, $err] = LuciferQuotesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new LuciferQuotesSpec(['step' => 's']),
            'response' => new LuciferQuotesResponse(self::resp(200, ['v' => 1])),
            'result' => new LuciferQuotesResult([]),
        ]);
        [, $err] = LuciferQuotesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new LuciferQuotesSpec(['step' => 's']),
            'response' => new LuciferQuotesResponse(self::resp(200, ['v' => 2])),
            'result' => new LuciferQuotesResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        LuciferQuotesMakeResponse::call($ctx);
    ...
    {
        $preset = new LuciferQuotesResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new LuciferQuotesSpec([]),
            'response' => new LuciferQuotesResponse([]),
            'result' => new LuciferQuotesResult([]),
        ...
        [$got, $err] = LuciferQuotesMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new LuciferQuotesResult([])]);
        [, $err] = LuciferQuotesMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new LuciferQuotesSpec([]), 'result' => null]);
        [, $err] = LuciferQuotesMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new LuciferQuotesSpec(['step' => 's']),
            'result' => new LuciferQuotesResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new LuciferQuotesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = LuciferQuotesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new LuciferQuotesSpec(['step' => 's']),
            'result' => new LuciferQuotesResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new LuciferQuotesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = LuciferQuotesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new LuciferQuotesResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new LuciferQuotesSpec([]),
            'result' => new LuciferQuotesResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = LuciferQuotesMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = LuciferQuotesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new LuciferQuotesError('boom', 'boom');
        $utility->fetcher = function (LuciferQuotesContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new LuciferQuotesSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = LuciferQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (LuciferQuotesContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = LuciferQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(LuciferQuotesResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        LuciferQuotesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (LuciferQuotesContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new LuciferQuotesSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = LuciferQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new LuciferQuotesResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new LuciferQuotesSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = LuciferQuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = LuciferQuotesMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = LuciferQuotesMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new LuciferQuotesResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, LuciferQuotesDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new LuciferQuotesResult(['ok' => false])]);
        $this->expectException(LuciferQuotesError::class);
        LuciferQuotesDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new LuciferQuotesResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', LuciferQuotesMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        LuciferQuotesMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new LuciferQuotesBaseFeature();
        $b = new LuciferQuotesBaseFeature();
        LuciferQuotesFeatureAdd::call($ctx, $a);
        LuciferQuotesFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): LuciferQuotesBaseFeature
    {
        $f = new LuciferQuotesBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        LuciferQuotesFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): LuciferQuotesContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new LuciferQuotesSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = LuciferQuotesPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        LuciferQuotesPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new LuciferQuotesResponse(['headers' => null]),
            'result' => new LuciferQuotesResult([]),
        ]);
        LuciferQuotesResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        LuciferQuotesResultBody::call($ctx);
