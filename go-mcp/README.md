...
[MCP](https://modelcontextprotocol.io) server exposing the LuciferQuotes SDK as
two agent tools — `lucifer-quotes_list` and `lucifer-quotes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/lucifer-quotes-mcp)
make build

# 2. Provide credentials via the environment
export LUCIFER_QUOTES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user lucifer-quotes \
  -- /absolute/path/to/lucifer-quotes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./lucifer-quotes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// lucifer-quotes_list: first page of records
{ "entity": "quote" }
{ "entity": "quote", "query": { } }

// lucifer-quotes_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/lucifer-quotes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user lucifer-quotes \
     -- "$PWD"/dist/*/lucifer-quotes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `lucifer-quotes_list` and `lucifer-quotes_load` tools now appear
   in new sessions. Ask the agent to *"list quote using lucifer-quotes"*
   and it calls `lucifer-quotes_list` with `{"entity":"quote"}`.
...
```sh
export LUCIFER_QUOTES_APIKEY=sk_live_xxx            # API key
export LUCIFER_QUOTES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `lucifer-quotes_list` tool
...
### Call the `lucifer-quotes_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `lucifer-quotes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `lucifer-quotes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LUCIFER_QUOTES_APIKEY` | API key sent with every request. |
| `LUCIFER_QUOTES_BASE` | Optional override of the API base URL. |
...
```sh
./lucifer-quotes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"lucifer-quotes_load","arguments":{"entity":"quote","query":{"id":1}}}}'
```
