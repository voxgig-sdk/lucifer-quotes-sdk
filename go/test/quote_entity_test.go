...
func quoteBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LUCIFER_QUOTES_TEST_QUOTE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LUCIFER_QUOTES_TEST_QUOTE_ENTID"])
	...
	if env["LUCIFER_QUOTES_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewLuciferQuotesSDK(core.ToMapAny(mergedOpts))
	}
