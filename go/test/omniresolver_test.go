...
import (
	...
	omni "github.com/voxgig-sdk/lucifer-quotes-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LuciferQuotesError); is {
				out := map[string]any{
					"name":    "LuciferQuotesError",
...
func omniCtx(arg any, client *sdk.LuciferQuotesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LuciferQuotesSDK); is {
			client = live
