...
// LuciferQuotes SDK retry feature
...
class LuciferQuotesRetryFeature extends LuciferQuotesBaseFeature
{
    ...
    public function init(LuciferQuotesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (LuciferQuotesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(LuciferQuotesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(LuciferQuotesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
