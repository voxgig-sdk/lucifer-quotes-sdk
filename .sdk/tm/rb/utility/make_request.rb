...
module LuciferQuotesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LuciferQuotesResponse.new({})
    ...
    if fetch_err
      ...
      response = LuciferQuotesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LuciferQuotesResponse.new(fetched)
    else
